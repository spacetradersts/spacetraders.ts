import { FlightPlansPayload, OrdersPayload, RawShip } from '../interfaces/APIPayload';
...
export class Ship extends BaseGameComponent {
    ...
    purchaseGoods(good: string, quantity: number) {
        return (this.client.request('POST', Endpoints.PURCHASE_ORDERS, { auth: true, params: { shipId: this.id, good, quantity } }) as Promise<OrdersPayload>)
        .then(data => {
    ...
    sellGoods(good: string, quantity: number) {
        return (this.client.request('POST', Endpoints.PURCHASE_ORDERS, { auth: true, params: { shipId: this.id, good, quantity } }) as Promise<OrdersPayload>)
        .then(data => {
            this.client.credits = data.credits;
            this.spaceAvailable = data.ship.spaceAvailable;
            const tempCargo = new Collection(data.ship.cargo.map(c => [c.good, new Cargo(this.client, c)]));
            if (tempCargo.has(good)) this.cargo.set(good, new Cargo(this.client, tempCargo.get(data.order.good)));
            else this.cargo.delete(good);
            return this;
        });
    }
    ...
    warp() {
        return (this.client.request('POST', Endpoints.WARP_JUMP, { auth: true, params: { shipId: this.id } }) as Promise<FlightPlansPayload>)
        .then(data => {
